...
const CouponsPage = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (selectedCoupon.times_used > 0) {
        toast({
          title: 'Warning',
          description: 'This coupon has been used. Deleting it may affect order history.',
          variant: 'default',
        });
      }
      ...
      if (error) {
        if (error.code === 'FK_VIOLATION') {
          toast({
            title: 'Error',
            description: 'Cannot delete this coupon as it is being used in orders.',
            variant: 'destructive',
          });
          return;
        }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all discount coupons across the platform
          </p>
        </div>
            ...
            <TableBody>
              {coupons.map((coupon) => (
                <TableRow key={coupon.id}>
                  ...
                  <TableCell>
                    {coupon.organiser?.email || 'Unknown'}
                  </TableCell>
                  <TableCell>
