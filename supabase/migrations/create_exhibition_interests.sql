...
DROP POLICY IF EXISTS "Brands can view their own interests" ON public.exhibition_interests;
DROP POLICY IF EXISTS "Brands can create interests" ON public.exhibition_interests;
DROP POLICY IF EXISTS "Organizers can view interests for their exhibitions" ON public.exhibition_interests;
...
CREATE POLICY "Brands can view their own interests"
...
CREATE POLICY "Brands can create interests"
    ON public.exhibition_interests
    FOR INSERT
    TO authenticated
...
CREATE POLICY "Organizers can view interests for their exhibitions"
    ON public.exhibition_interests
    FOR SELECT
    TO authenticated
    USING (EXISTS (
...
DROP INDEX IF EXISTS idx_exhibition_interests_exhibition_id;
DROP INDEX IF EXISTS idx_exhibition_interests_brand_id;
...
-- Grant permissions
GRANT SELECT, INSERT ON public.exhibition_interests TO authenticated; 